...
export default function Perusahaan() {
  ...
  const [isPanduan, setIsPanduan] = useState(false);
...
const PerusahaanCard = ({ data, selectedId, setSelectedId, setIsPanduan }) => {
  const {
    id,
    location,
    postedAt,
    ...
    >
      <Card
        className={`gap-4 w-[100%] ${
          id == selectedId && "xl:border-primaryBlue xl:border-[2px]"
        }`}
      >
...
const CardLowongan = ({ selectedId, setIsPanduan, isPanduan }) => {
  let [searchParams] = useSearchParams();
  const selectedJob = searchParams.get("selectedJob");
  const navigate = useNavigate();

  return (
    <Card
      className={`${
        selectedJob ? "fixed inset-0 xl:block xl:relative rounded-none" : "relative hidden"
      } flex-1 xl:block  gap-4 overflow-y-scroll z-[10000]`}
    >
      <button className="lg:hidden" onClick={() => navigate(location.pathname)}>
        <RxCross1 />
      </button>
      {isPanduan ? (
        <PanduanLowongan selectedId={selectedId} setIsPanduan={setIsPanduan} />
      ) : (
        <DetailLowongan selectedId={selectedId} setIsPanduan={setIsPanduan} />
      )}
    </Card>
  );
};

const DetailLowongan = ({ selectedId, setIsPanduan }) => {
  const navigate = useNavigate();
  return (
    <div className="space-y-4">
      <div className="flex gap-4 items-center justify-between w-full">
        ...
        <div className="flex flex-col gap-1">
          ...
          <Button onClick={() => setIsPanduan(true)}>
            Panduan <FiBook />
          </Button>
        </div>
      </div>
      ...
      <ul>
        ...
        <li className="flex items-center gap-2">
          <FiGlobe className="text-xl" /> Advertising, Digital Marketing, Social
          Media
        </li>
      </ul>
      ...
      <p>
        ...
        mengumpulkan wawasan untuk perbaikan tindakan selanjutnya. Bekerja sama
        dengan pemangku kepentingan lainnya mulai dari pemasaran merek, produk,
        dan urusan korporat hingga layanan pelanggan untuk mendukung kebutuhan
        mereka melalui ranah media sosial. Anda akan menjadi orang media sosial
        yang bertanggung jawab atas segala kebutuhan dan mengukur dampak hasil
        ini terhadap OKR.
      </p>
      ...
      <ul className="list-disc pl-4">
        ...
        <li>
          Kemampuan mengkomunikasikan ide dengan jelas; pola pikir analitis dan
          berbasis data yang kuat
        </li>
        <li>
          Pengetahuan luas tentang lanskap, platform, dan teknologi media sosial
        </li>
        <li>
...
const PanduanLowongan = ({ selectedId, setIsPanduan }) => {
  return (
    <div className="space-y-4">
      <div className="flex gap-4 items-center w-full justify-between">
        ...
        <Button variant="default" onClick={() => setIsPanduan(false)}>
          Kembali
        </Button>
      </div>
